 ...
 *
 * Handles the prefixes we see in practice - "Rs.", "Rs ", "INR ", "INR" - and strips
 * the thousands separators before handing back a BigDecimal with scale 2.
 */
public final class Amounts {
    ...
    private static final Pattern BALANCE = Pattern.compile(
            "(?:Avl\\s*Bal|Available\\s*Balance|BalAvl|Avl\\s*Limit|Available\\s*Limit)\\s*:?\\s*"
                    + "(?:Rs\\.?|INR)?\\s*([0-9,]+(?:\\.[0-9]{1,2})?)",
            Pattern.CASE_INSENSITIVE);
